...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Master localization file is German, it is used when key uses natural text in German to lookup
  proper key -->
  ...
  <xsl:variable name="l10n-doc">
    <xsl:choose>
      ...
      <xsl:when test="doc-available($l10n-master-filename)">
        ...
        <!--<xsl:message>Unable
        to find localization for {$lang}. Using default from de.xml.</xsl:message>-->
      </xsl:when>
      <xsl:otherwise>
        <!--<xsl:message>Unable
        to find localization for {$lang}. Can't load default from de.xml. Using empty
        localization.</xsl:message>-->
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
    ...
    <xsl:choose>
      ...
      <xsl:when test="$l10n-nl-lookup">
        ...
        <xsl:variable name="key2"
          select="(key('l10n-key', $key, $l10n-master-doc)/@key)[1]" />
        <xsl:variable
          name="localized2" select="key('l10n', $key2, $l10n-doc)" />
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            ...
            <!--<xsl:message>Unable
            to find localization for {$key}.</xsl:message>-->
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:function name="xrf:get-id" as="xs:string">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:sequence select="'???'" />
        <!--<xsl:message>Unable
        to find BT/BG id for {$key}.</xsl:message>-->
      </xsl:otherwise>
    </xsl:choose>
  </xsl:function>

  <!-- We are emulating older template for getting labels in order to maintain backward
  compatability -->
  <xsl:template name="field-mapping">
    <xsl:param name="identifier" />
    
    <label>
      <xsl:value-of select="xrf:_($identifier)" />
    </label>
    <nummer>
      <xsl:value-of select="xrf:get-id($identifier)" />
    </nummer>
  </xsl:template>
  ...
  <xsl:function name="xrf:handle-specification-identifier" as="xs:string">
    ...
    <xsl:choose>
      <xsl:when test="matches($specification-identifier, 'xrechnung_[0-9]\.[0-9]#conformant')">
        <xsl:value-of
          select="replace($specification-identifier, '(xrechnung_[0-9]\.[0-9])(#conformant)', '$1 $2')" />
      </xsl:when>
      <xsl:otherwise>
