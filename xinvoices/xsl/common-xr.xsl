...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="date">
        ...
        <xsl:variable
            name="datematch" select="boolean(normalize-space($normalizeddate))" />
        <xsl:variable
            name="yearstring" select="substring($normalizeddate, 1, 4)" />
        ...
        <xsl:variable
            name="monthstring" select="substring($normalizeddate, 5, 2)" />
        <xsl:variable
            name="month">
            <xsl:choose>
                <xsl:when
                    test="string-length($monthstring) = 2 and number($monthstring) > 0 and number($monthstring) &lt; 13">
        ...
        <xsl:variable
            name="daystring" select="substring($normalizeddate, 7, 2)" />
        <xsl:variable name="day">
            <xsl:choose>
                <xsl:when
                    test="string-length($daystring) = 2 and number($daystring) > 0 and number($daystring) &lt; 32">
        ...
        <xsl:choose>
            <xsl:when test="$year &gt; 0 and $month &gt; 0 and $day &gt; 0">
                <xsl:value-of
                    select="concat(format-number($year,'0000'), '-', format-number($month, '00'), '-', format-number($day, '00'))" />
            </xsl:when>
